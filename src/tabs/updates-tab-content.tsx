...
import { getSourceLabel, SourceBadge, SourceBadgeLegend } from '../components/source-badge';
...
export function UpdatesTabContent({ loading, error, report, lastUpdated, onRefresh, onDownloadJson, canDownload }: UpdatesTabProps) {
    ...
    const sourceLegendItems = useMemo(() => {
        ...
        return [...sources].sort((a, b) => getOrder(a) - getOrder(b) || a.localeCompare(b));
    }, [sourceOptions]);
                ...
                <ToolbarContent>
                    <ToolbarItem>
                        <SearchInput
                            ...
                            aria-label={_("Filter findings by package ID or summary")}
                    ...
                    <ToolbarItem>
                        <select className="pf-v6-c-form-control" aria-label={_("Filter by source")} value={sourceFilter} onChange={event => setSourceFilter(event.currentTarget.value)}>
